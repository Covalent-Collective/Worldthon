...
## Flow 5: 대시보드 (DASHBOARD `/dashboard`)
...
**Dashboard 핵심 요소:**
- Stats Row: 기여 노드 수, 총 인용 횟수 (font-digital)
- Activity Heatmap: 66일 히트맵 (22열 × 3행), 4단계 밝기
- 참여 커뮤니티: 3개 고정, 프로필 이미지 + 노드 수, 클릭 시 커뮤니티 상세로 이동
- Recent Contributions: 타임라인 UI, 아이콘 + 라벨 + 상대시간

---

## Flow 6: 커뮤니티 상세 (COMMUNITY `/community/[id]`)
...
│  "창업, 투자, 스케일업 경험 공유" │
│                                 │
│  [멤버 15] [노드 342] [인용 1.2K]│
│                                 │
│  ▶ GRAPH VIEW                  │  <- 아코디언: 클릭 시 그래프 펼침
│  ┌─────────────────────────────┐│
│  │ [KnowledgeGraph canvas]     ││
│  └─────────────────────────────┘│
│                                 │
│  ▶ PUBLISH BOT (출봇 투표)      │  <- 아코디언
│  ┌─────────────────────────────┐│
│  │ v2.4 출봇 투표              ││
│  │ ■■■■■■■□□□ 찬성 68%        ││
│  │ ■■□□□□□□□□ 반대 18%        ││
...
│  │ Week 4  1/27 - 2/2         ││
│  │ [img][img][img][img][img]   ││  <- 캡슐 형태 (첫: 왼 둥글, 끝: 오른 둥글)
│  │                             ││
│  │ Week 3  1/20 - 1/26        ││
│  │ [img][img][img][img][img]   ││
│  └────────────────────────────┘│
...
**Community Detail 핵심 요소:**
- 프로필 헤더: 이미지 + 이름 + 설명 + 카테고리
- 스탯 칩: 멤버, 노드, 인용 (font-digital)
- Graph View 아코디언: AnimatePresence로 KnowledgeGraph 펼침/접기
- PUBLISH BOT 아코디언: 출봇 투표 UI (찬성/반대 프로그레스 바, 정족수, 투표 버튼)
- Weekly Photo Archive: 주간 사진 (Week 4→1 역순), 캡슐 카드 형태, picsum.photos 이미지

---

## Flow 7: Explore 상세 인용 시스템
...
│  [반투명 backdrop]              │
│  ┌─────────────────────────┐    │
│  │ [1] MVP 개발 전략    X │    │  <- 전체화면 중앙 모달
│  │                        │    │
│  │ 원본 문서              │    │
│  │ ...이전 텍스트         │    │
│  │ [하이라이트된 인용 부분] │   │  <- bg-aurora-cyan/20
│  │ 이후 텍스트...         │    │
│  │                        │    │
│  │ 배고픈 호랑이 | 인용 45회│   │
│  │               2025.09.10│   │
...
**인용 시스템 핵심 요소:**
- 인용 마커: 각 문단 끝에 `[n]` 표시 (cyan 색상, superscript)
- 전체화면 모달: fixed 중앙 배치, scale 애니메이션
- 원본 문서 뷰: 전후 맥락 + 하이라이트된 인용 부분
- 기여자 정보: 익명 닉네임 + 인용 횟수 + 날짜
- 질문 답변에서도 동일한 모달 사용
...
- 5개 탭: Journal (마이크), Explore (지구), Community (사람들), Dashboard (차트), Reward (선물)
...
┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
│  Journal     │──│  Explore     │──│  Community   │──│  Dashboard   │──│  Reward      │
│  (음성 저널링)│  │  (지식 탐색)  │  │  (커뮤니티)   │  │  (대시보드)   │  │  (보상 확인)  │
└──────┬───────┘  └──────┬───────┘  └──────┬───────┘  └──────┬───────┘  └──────────────┘
       │                 │                 │                 │
       ▼                 ▼                 ▼                 ▼
┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
│  녹음        │  │  Vault 선택   │  │  커뮤니티 상세│  │  히트맵      │
│  (VoiceOrb)  │  │  (3D Carousel)│  │  (/community) │  │  + 최근 기여  │
└──────┬───────┘  └──────┬───────┘  └──────┬───────┘  └──────────────┘
       │                 │                 │
       ▼                 ▼                 ▼
┌──────────────┐  ┌──────────────┐  ┌──────────────┐
...
- 5개 주요 탭: Journal / Explore / Community / Dashboard / Reward (하단 네비게이션으로 자유 이동)
- Journal 흐름: 녹음 -> AI 분석 -> Vault 매칭 -> 기여 완료
- Explore 흐름: Vault 카드 탐색 -> 상세 -> 질문 -> 답변 + 인용 마커 모달
- Community 흐름: 커뮤니티 목록 -> 상세 (그래프 뷰, 출봇 투표, 주간 사진)
- Dashboard 흐름: 활동 히트맵 + 참여 커뮤니티 + 최근 기여 타임라인
